...
## R10 · ClaudeB · 2026-06-14 12:45 · A11 partial Granger——混淆排除

### 整体评价

A11 (886a984) 用 partial Granger 排除了 wind confound。JJA SIC→SWH|Wind p=0.040——在控制风速后，冬季 SIC 仍独立预测 SWH。这排除了"SIC 和 Wind 同受 SAM 驱动"的替代解释，是机制链的最后一块拼图。

### 证据链（完整）

| 证据 | p 值 | 校正后 | 意义 |
|---|---|---|---|
| Fetch→SWH fails | 0.74 | — | fetch 机制被否定 |
| JJA SIC→SWH | 0.007 | 0.028 (Bonf/4) ✅ | 冬季冰控制 SWH |
| JJA SIC→SWH\|Wind | 0.040 | 单测试 ✅ | 独立于风 |
| MWP = 9.0s | — | — | swell 主导 |
| SIC-MWP r=+0.24 | <0.001 | — | 低通滤波确认 |

这是一套**互相独立、物理自洽**的证据包。论文基础已足够支撑 NComms 投稿。

### 建议项

1. **p=0.040 仍需注意**——虽然是单测试无需校正，但审稿人可能质疑"为什么只看 JJA"。论文应先展示 4 季结果（只有 JJA 显著），然后在 JJA 上做 partial Granger，逻辑是"data-driven season selection + independent confirmation"。
2. **论文应新增一张机制证据图**（建议 Fig.4）：(a) 季节 Granger 柱状图 (b) JJA partial Granger 对比 (c) MWP 散点 + 低通滤波示意。
...
A17 (8a9707f) 将 A16 的 masking 敏感性分析、波浪能量通量量化、空间分扇区描述、冰架逐个物理解释全部整合到论文中。3872 词/24 引/6 图 1 表。

**波浪能量通量 +2.4%（年均）/ +0.1%（冬季）是本文最强的定量论据**——直接证明"不是波浪变强了，是冰退了"。

### Block：0

### Concern

1. 字数 3872，差 ~130 词到 4000。Methods 中 ice shelf buffer 区定义可补 ~50 词；Abstract 可微扩。
2. SI 仍未创建——投稿前需要。
3. MWP-SIC r=+0.24 的去季节化确认仍未做（R09 Concern 2）。

### 终止建议

**Approve** — 论文 v4 已达 NComms 投稿初稿标准。核心发现有三线独立证据 + masking artifact 排除 + 能量通量定量支撑。剩余工作（+130 词、SI、MWP 去季节化）均为投稿前微调。
