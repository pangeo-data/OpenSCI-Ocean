...
\abstract{
Topological theory predicts that equatorial Kelvin and Yanai waves are interface modes protected by the sign reversal of the Coriolis parameter across the equator, with the bulk Poincar\'e wave bands carrying non-trivial Chern numbers of $\pm 2$. Whether this topological protection leaves observable dynamical fingerprints in the turbulent, forced, and dissipative real ocean has remained untested. Here we develop an observational framework combining multi-mission altimetry (DUACS) event tracking with SWOT wide-swath snapshots to test whether equatorial Kelvin waves retain topological-like robustness in the real ocean. From DUACS daily SSH, we identify 7 independent Kelvin wave events in the tropical Pacific during 2023--2024. Ray-following diagnostics reveal perturbation-type dependence in amplitude retention: preservation at island chains, loss in tropical instability wave (TIW) regions, though comparisons against three control groups do not reach significance at the 7-event sample size. A simple amplitude-based gap-to-perturbation ratio $\Lambda$ fails to distinguish perturbation zones in real data at any spatial scale --- island wakes and TIW vortex cores have similar local vorticity but produce opposite wave responses. A resonance-window reformulation $\Lambda_2$ resolves this by targeting the physically relevant scattering channel: TIWs carry energy at the wavenumber--frequency connecting Kelvin modes to the westward wave band, while island wakes are wavenumber-mismatched by an order of magnitude. Within TIW zones, $\Lambda_2$ shows a suggestive per-event correlation with observed amplitude loss ($r = 0.69$, $p = 0.09$, $n = 7$), consistent with triad resonant scattering as the dominant breakdown mechanism. These results establish the observational and diagnostic infrastructure for testing topological wave theory in the ocean and identify resonant coupling --- not perturbation amplitude --- as the controlling factor for protection breakdown.
}
...
An initial formulation using $\delta\omega_\mathrm{pert} = \max(|\zeta|/2,\ |U_0 k_x|)$ from GLORYS12 surface currents yields zone-averaged $\Lambda$ of $6.2 \pm 0.8$ (Gilbert Islands), $5.8 \pm 0.6$ (Line Islands), and $6.5 \pm 1.0$ (TIW zone) --- no significant inter-zone variation, and all well above $\Lambda_c \sim 1$. When the same quantity is computed along the Kelvin ray using local-maximum vorticity (0.5$^\circ$ smoothing), all three zones collapse to $\Lambda_\mathrm{min} \approx 1$ (Gilbert $1.25 \pm 0.42$; Line $0.97 \pm 0.17$; TIW $1.16 \pm 0.33$), and the ranking reverses: Line Islands has the lowest $\Lambda_\mathrm{min}$ despite the highest amplitude retention. Island wakes and TIW vortex cores have similar local $|\zeta| \sim 1.5 \times 10^{-5}$~s$^{-1}$, but waves respond to them differently. The amplitude-based criterion cannot distinguish these perturbation types at any spatial averaging scale.

\paragraph{Resonance-window criterion (V2).}
The failure of the amplitude criterion points to a structural distinction: the spectral gap protects against elastic (frequency-conserving) scattering, but not against resonant triad coupling. Static perturbations (island wakes) carry their enstrophy at scales of $10$--$100$~km, requiring a wavenumber transfer $\Delta k \approx 6 \times 10^{-6}$~m$^{-1}$ (${\sim}1{,}000$~km) to connect the intraseasonal Kelvin mode ($k \approx +1 \times 10^{-6}$~m$^{-1}$) to the $n = 1$ Rossby branch ($k \approx -5 \times 10^{-6}$~m$^{-1}$) --- a mismatch of an order of magnitude. TIWs ($\lambda \approx 1{,}100$~km, westward, period $20$--$40$~days) sit at the $(k, \omega)$ that connects Kelvin modes to the westward wave band, enabling near-resonant triad coupling\cite{Lyman2005}.

We define a revised parameter $\Lambda_2 = \Delta\omega_\mathrm{eff} / \delta\omega_\mathrm{res}$, where $\delta\omega_\mathrm{res}$ is estimated from the vorticity power within the resonant $(k, \omega)$ window (westward phase, wavelengths $700$--$2{,}500$~km, periods $15$--$50$~days), weighted by the Kelvin meridional footprint (see Methods; Eq.~\ref{eq:lambda2}). The resonant fraction of total vorticity variance distinguishes zones: $0.13 \pm 0.06$ in TIW zones versus $0.05 \pm 0.03$ at island chains. Within TIW zones, the resonant fraction captures the seasonal cycle of TIW activity (boreal spring ${\sim}0.03$, winter ${\sim}0.20$), providing within-zone variance that the zone-mean approach cannot.

\paragraph{Validation against observed amplitude retention.}
Fig.~\ref{fig:lambda} shows $\Lambda_2$ versus the observed amplitude ratio. Across all event--zone pairs ($n = 14$), Spearman $\rho = 0.42$ ($p = 0.14$), not reaching significance. However, the resonance mechanism is expected to dominate only in the TIW zone where the coupling channel is active; at island chains, waveguide focusing controls amplitude retention independently of $\Lambda_2$. Within the TIW zone ($n = 7$), Pearson $r = 0.69$ ($p = 0.086$) and Spearman $\rho = 0.64$ ($p = 0.12$). As a negative control, the Line Islands show no significant correlation ($\rho = -0.54$, $p = 0.27$, $n = 6$). The TIW-zone correlation is suggestive but does not reach significance at the $\alpha = 0.05$ level; a larger event catalog ($\geq 20$ events spanning multiple ENSO cycles) is needed for a definitive test.
...
\caption{\textbf{Resonance-window control parameter $\Lambda_2$.} \textbf{a}, $\Lambda_2 = \Delta\omega_\mathrm{eff} / \delta\omega_\mathrm{res}$ by perturbation zone. Horizontal bars indicate zone means. TIW-zone values span a wider range than island zones, reflecting the seasonal modulation of TIW activity. \textbf{b}, $\Lambda_2$ versus observed amplitude ratio (DUACS ray-following) for each event--zone pair with upstream signal $> 0.01$~m. Overall Spearman $\rho = 0.42$ ($p = 0.14$, $n = 14$); within the TIW zone where the resonant coupling channel is active, Pearson $r = 0.69$ ($p = 0.09$, $n = 7$). Dotted line marks amplitude ratio $= 1$ (no change).}
\label{fig:lambda}
\end{figure}
...
The framework established here --- event detection, deduplication, multi-control robustness diagnostics, and the resonance-window $\Lambda_2$ parameter --- is ready to be applied to the full 30-year altimetric record (1993--present) as a pre-registered hypothesis test to achieve the statistical power needed for definitive conclusions about topological protection in the ocean.
