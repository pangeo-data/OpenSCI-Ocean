...
\abstract{
Topological theory predicts that equatorial Kelvin and Yanai waves are interface modes protected by the sign reversal of the Coriolis parameter across the equator, with the bulk Poincar\'e wave bands carrying non-trivial Chern numbers of $\pm 2$. Whether this topological protection leaves observable dynamical fingerprints in the turbulent, forced, and dissipative real ocean has remained untested. Here we develop an observational framework combining multi-mission altimetry (DUACS) event tracking with SWOT wide-swath snapshots to test whether equatorial Kelvin waves retain topological-like robustness in the real ocean. From DUACS daily SSH, we identify 7 independent Kelvin wave events in the tropical Pacific during 2023--2024 (via ridge-intercept clustering of candidate rays). Ray-following diagnostics reveal perturbation-type dependence in amplitude retention: Kelvin waves tend to maintain or enhance amplitude at island chains but lose energy in tropical instability wave regions. However, statistical comparisons against three control groups (westward Rossby rays, stationary reference, and time-shifted placebo) do not reach significance at the 7-event sample size (block bootstrap $p > 0.05$ for all comparisons). We propose an effective gap-to-perturbation frequency ratio $\Lambda = \Delta\omega_\mathrm{eff} / \delta\omega_\mathrm{pert}$ ($\Delta\omega_\mathrm{eff} = \sqrt{\beta c_1} \approx 7.6 \times 10^{-6}$~s$^{-1}$) as a framework for predicting where topological protection should hold ($\Lambda \gg 1$) and break down ($\Lambda \sim 1$). These results establish the observational and diagnostic infrastructure for testing topological wave theory in the ocean and demonstrate that robust attribution requires larger event catalogs spanning multiple ENSO cycles.
}
...
\textbf{c}, [NOTE: panel uses preliminary hardcoded values; real data from ray-following analysis yields Line Islands $2.0 \pm 0.7$, TIW zone $0.7 \pm 0.5$, Gilbert Islands $11.4 \pm 8.8$ ($n = 2$, not robust). Panel to be regenerated with real data.]}
\label{fig:hovmoller}
\end{figure}
...
The results reveal a clear dependence on perturbation type (Fig.~\ref{fig:hovmoller}c). At the Line Islands, the Kelvin wave amplitude ratio is $2.0 \pm 0.7$ (mean $\pm$ s.d., $n = 7$ event--zone measurements), indicating approximate amplitude preservation. In the TIW-active zone, the ratio drops to $0.7 \pm 0.5$ ($n = 7$), indicating net energy loss. At the Gilbert Islands, only 2 valid measurements were obtained, with anomalously high ratios ($11.4 \pm 8.8$) likely reflecting waveguide focusing in a narrow topographic constriction rather than a statistically robust signal. For comparison, the westward Rossby control shows $1.0 \pm 0.6$ ($n = 14$), the stationary control $2.1 \pm 1.4$ ($n = 21$), and the time-shifted placebo $1.2 \pm 0.7$ ($n = 16$). Block bootstrap tests (events as blocks, $n = 10{,}000$ resamples) show no significant differences between Kelvin and any control group ($p > 0.05$ for all comparisons), indicating that the current 7-event sample is insufficient for definitive statistical attribution.
...
Our preliminary results are consistent with the hypothesis that topological protection of equatorial Kelvin waves\cite{Delplace2017} is conditional rather than absolute, but do not yet constitute definitive proof. With only 7 independent events from a single El Ni\~no episode, no pairwise comparison against control groups reaches statistical significance (block bootstrap $p > 0.05$). Nevertheless, the pattern of perturbation-type dependence in amplitude retention---preservation at island chains, loss in TIW zones---motivates the proposed gap-to-perturbation ratio $\Lambda$ as a predictive framework. When $\Lambda \gg 1$, Kelvin waves are expected to propagate with preserved amplitude and unidirectional character; when $\Lambda \sim 1$, energy loss and mode conversion become likely.
...
Fourth, $\Lambda$ estimates rely on reanalysis-derived vorticity (GLORYS12) and Argo-derived stratification, both carrying uncertainties of order 20--30\% in the equatorial waveguide. The $\Lambda_c \sim 1$ threshold is a prediction from this framework, not yet observationally confirmed.
...
\begin{enumerate}
\item Kelvin wave amplitude retention shows a perturbation-type dependence consistent with conditional robustness: higher at island chains, lower in TIW zones.
\item However, no pairwise comparison reaches statistical significance at the current 7-event sample size (block bootstrap $p > 0.05$).
\item Spectral decomposition (with corrected FFT sign convention, detrending, and 90-day highpass) reveals that Kelvin modes account for 41.6\% of the intraseasonal SSH variance in the equatorial Pacific during 2023--2024.
\item We propose the gap-to-perturbation frequency ratio $\Lambda = \sqrt{\beta c_1} / \delta\omega_\mathrm{pert}$ as a quantitative framework for predicting where topological protection should hold and where it should fail.
\end{enumerate}

The framework established here---event detection, deduplication, multi-control robustness diagnostics, and the $\Lambda$ parameter---is ready to be applied to the full 30-year altimetric record (1993--present) to achieve the statistical power needed for definitive conclusions about topological protection in the ocean.
