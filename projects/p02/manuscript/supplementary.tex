...
\paragraph{Record-length caveat.} Event windows shorter than twice the longest resonant period ($2 \times 50$~days) resolve that period with fewer than two frequency bins. This affects one event (KE05, 50-day window), flagged as \texttt{window\_caveat} in the released data. Excluding KE05, the TIW-zone correlation retains its sign and approximate magnitude (Pearson $r = 0.63$ at $n = 6$, versus $r = 0.69$ at $n = 7$), although the rank correlation weakens at this sample size (Spearman $\rho = 0.43$ versus $0.64$).
